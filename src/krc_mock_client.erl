...
handle_call(connect, _, #s{con = false} = S) ->
    {reply, ok, S#s{con = true}};
handle_call(disconnect, _, #s{con = true} = S) ->
    {reply, ok, S#s{con = false}};
handle_call({lag, Ms}, _, #s{} = S) ->
    {reply, ok, S#s{lag = Ms}};
handle_call({F, A}, _, #s{} = S0) ->
    {S, Ret} = do_call(F, A, S0),
    {reply, Ret, S};
        ...
        case s2_maps:get(Idxs, [B, I]) of
            {ok, Dict} ->
                lists:foldl(
                    ...
                    [],
                    [K || K <- dict:fetch_keys(Dict), L =< K, K =< U]
                );
            {error, notfound} ->
                {ok, []}
        end};
